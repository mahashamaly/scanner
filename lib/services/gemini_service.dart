...
class GeminiService {
  ...
  final String _apiKey = 'AIzaSyBbUjrjnG43rJKlUz6kB0BZ7qJcLvz1D5w';
...
اعتبر الوثيقة "مراسلة داخلية" فقط إذا تحقق ما يلي:

1️⃣ الترويسة:
- وجود شعار بلدية غزة.
- وجود اسم إدارة محددة (مثل: الإدارة العامة للشؤون الإدارية / المالية / الهندسية ...).

2️⃣ أطراف المراسلة:
- وجود صيغة تخاطب داخلية مثل:
  - "إلى السيد..."
  - "إلى رئيس البلدية..."
  - "إلى مدير عام..."
- وجود صيغة "من:" أو توقيع مسؤول إدارة داخلية.

3️⃣ الموضوع:
- وجود كلمة "الموضوع:" متبوعة بنص إداري واضح.

4️⃣ التأشيرات:
- وجود خطوط يد أو هوامش أو تواقيع متعددة من مدراء مختلفين على نفس الورقة.

⚠️ إذا كانت الوثيقة:
- "قرار تعيين"
- "تعديل حالة اجتماعية"
- أو أي قرار إداري رسمي مستقل

فلا تصنفها كمراسلة داخلية، :

---

ثانياً: إذا تأكدت أنها مراسلة داخلية، استخرج الحقول التالية فقط:
 1️⃣ correspondence_number:
- الرقم المكتوب بجانب كلمة "الرقم".
- أو الرقم المكتوب يدوياً في أعلى الصفحة بجانب:
  - "ص"
  - "صادر"
- لا تستخرج أي أرقام أخرى غير رقم المراسلة الرسمي.

2️⃣ correspondence_date:
- التاريخ الرسمي للمراسلة.
- ابحث عنه بجانب كلمة "التاريخ".
- أو في ختم "صادر" العلوي.
- لا تخمن أي تاريخ آخر.

3️⃣ correspondence_title:
- النص المكتوب بالكامل بجانب كلمة "الموضوع:".
- اكتبه كما هو بدون اختصار أو تعديل.



⚠️ قواعد مهمة:
- لا تخمن أي قيمة.
- إذا الحقل غير واضح أو غير موجود أعده null.
- لا تستخرج بيانات غير مطلوبة.

أعد النتيجة بصيغة JSON فقط بالشكل التالي:

إذا كانت مراسلة:
{
  "document_type": "مراسلة داخلية",
  "correspondence_date": "...",
...
1) ابحث فقط عن عبارة "رقم الكشف" أو "الرقم" في الترويسة العليا.
2) استخرج الرقم المرتبط بهذه العبارة فقط وضعه في correspondence_number.
3) لا تستخدم أبداً أي أرقام أخرى مثل:
   - رقم الكمبيوتر
   - رقم الهوية
   - رقم الملف
   - أي رقم موجود داخل الجدول
4) إذا لم يوجد "رقم الكشف" في الترويسة، أعد القيمة فارغة:
   "correspondence_number": ""



مهم جداً:
اعتبر المستند من فئة "كشف بيانات" حتى لو لم تكن العبارة مكتوبة حرفياً، طالما تنطبق عليه المواصفات السابقة.
